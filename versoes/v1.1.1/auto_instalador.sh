...
echo -e "Se você não conseguir acessar os serviços, execute este comando para diagnosticar problemas:"
echo -e "   ${AMARELO}./$(basename $0) --diagnostico${NC}"
echo -e ""
...
verificar_conectividade() {
  ...
  echo -e "4. ${BRANCO}Se nada funcionar:${NC} Verificar o painel de controle do seu provedor VPS para regras de firewall adicionais"
  echo -e ""
...
# No final do script, após a mensagem de sucesso, vamos adicionar instruções específicas sobre firewalls de VPS

echo -e "${VERDE}=== VERIFICAÇÃO DE FIREWALL DO PROVEDOR VPS ===${NC}"
echo -e "Muitos provedores VPS têm firewalls adicionais que precisam ser configurados:"
echo -e ""
echo -e "${BRANCO}AWS:${NC} Verifique 'Security Groups' e adicione regras de entrada para portas 80 e 443"
echo -e "${BRANCO}DigitalOcean:${NC} Vá para 'Networking' > 'Firewalls' e adicione regras de entrada"
echo -e "${BRANCO}Linode:${NC} Acesse 'Network' > 'Firewall' e configure as portas necessárias"
echo -e "${BRANCO}Vultr:${NC} Vá para 'Firewall' e crie regras para permitir HTTP e HTTPS"
echo -e "${BRANCO}Contabo:${NC} Verifique as configurações de firewall no painel de controle"
echo -e "${BRANCO}OVH:${NC} Configure o firewall nas 'Network Interfaces' do seu VPS"
echo -e ""
echo -e "Se após verificar as configurações do provedor ainda não conseguir acessar os serviços, execute:"
